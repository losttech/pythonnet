...
", null, locals.Handle);
            ...
            Assert.IsFalse(codec.CanDecode(fooFunc, typeof(Action<bool>)));
            ...
            Assert.IsTrue(codec.CanDecode(barFunc, typeof(Action<bool>)));

            Action<bool> barAction;
            Assert.IsTrue(codec.TryDecode(barFunc, out barAction));
            Assert.DoesNotThrow(() => barAction(true));
