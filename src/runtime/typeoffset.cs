...
namespace Python.Runtime
{
    ...
    internal class TypeOffset
    {
        static TypeOffset()
        {
            string versionedOffsetTypeName = typeof(TypeOffset).FullName + Runtime.pyver;
            var versionedOffsetType = typeof(TypeOffset).Assembly.GetType(versionedOffsetTypeName);
            if (versionedOffsetType == null)
                throw new NotSupportedException("Python version is not supported");

            FieldInfo[] fi = versionedOffsetType.GetFields();
